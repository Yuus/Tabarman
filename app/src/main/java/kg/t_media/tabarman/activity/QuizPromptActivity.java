...
public class QuizPromptActivity extends BaseActivity {
    ...
    private void initVar() {
        ...
        if (intent != null) {
            categoryId = "1";//intent.getStringExtra(AppConstants.BUNDLE_KEY_INDEX);
            score = "0";//AppPreference.getInstance(mContext).getString(categoryId);
            questionsCount = "2";//AppPreference.getInstance(mContext).getString(categoryId + AppConstants.QUESTIONS_IN_TEST);
        }
    }
...
//                ActivityUtilities.getInstance().invokeCommonQuizActivity(mActivity, QuizActivity.class, categoryId, true);
        ...
        mBtnNo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ActivityUtilities.getInstance().invokeNewActivity(mActivity, Quest.class, true);
            }
        });
    }
